...
namespace GreenBeret
{
    ...
    void Map::Render()
    {
        for (const auto &polygon : volFile->polygons)
        {
            ...
            // TODO: tiles may not be drawn every frame
            // draw tiles
            for (const auto &tile : polygon->tiles)
            {
                if (tile->fileName.at(0) != '-') // sprites starting with "minus" are invisible
                {
                    SDL_Point position = (SDL_Point)(tile->position - Map::Get()->offset);
                    SDL_Point size = (SDL_Point)tile->size;
                    SDL_Rect mapRect{position.x, position.y, size.x, size.y};
                    SDL_RenderCopy(Window::Get()->renderer, tile->texture, nullptr, &mapRect);

                    // red
                    SDL_SetRenderDrawColor(Window::Get()->renderer, 255, 0, 0, 255);
                    // draw tile rectangle
                    //SDL_RenderDrawRect(Window::Get()->renderer, &mapRect);
                }
            }
        }
